        ...
        secondary={
          <React.Fragment>
            <Typography
              ...
              color="text.primary"
            />
            <span>{item.Words.length} total</span>
  ...
  useEffect(() => {
    if (filter !== undefined) {
      let _groups = dataGroups.filter((obj) => {
        let _filter = obj.Words.filter((_) =>
          filterWordByType(userInfo.FirstShowed ? "Name" : "Value", _, filter)
        );
    ...
    <div>
      ...
      <div>
        {isLoading ? (
          ...
          <div>{GroupListComponent(groups, filter, deleteGroup)}</div>
        )}
      </div>
      <div>
