...
export const PlaySpace = () => {
  ...
  const [isHistorifyMessageEnable, setIsHistorifyMessageEnable] =
  ...
  const nextValue = () => {
    let wordsFilterd = result.Words.filter(
      (_) => !_.IsKnowed && !_.Reveled && _.Cycles == currentCycle
    );
    ...
    if (nextElement) {
      ...
      if (currentCycle >= 3) {
        ...
        updateWords
          .filter((_) => _.Cycles === currentCycle && !_.IsKnowed)
          .forEach((_) => {
            _.Reveled = false;
            _.Cycles++;
          });
  ...
  const getData = async () => {
    let group = (await Adapter.getGroup(
      userInfo.UserId,
      userInfo.PlayingGroup.toString()
    )) as IGroup;
  ...
  useEffect(() => {
    if (userInfo.PlayingGroup == "0") navigate(`/groups`);
    else {
      setCurrentCycle(0);
      getData();
    }
  ...
  if (indexWord < 0 && !isVeryEndedCycle) return <div>Loading..</div>;
      ...
      <div>
        ...
        {isVeryEndedCycle && (
          <Alert severity="success">
            <AlertTitle>
              Great Gig!, you learned {summary.Learned} and attain an{" "}
              {Math.ceil((summary.Learned / summary.Total) * 100)} of progress
            </AlertTitle>
